...
//Institution: Universidade Federal de Minas Gerais and ITV (Instituto Tecnológico Vale)
//Description: This file This file constructs the map-cloud. This is a modification of original ROS package LeGO-LOAM, where
//             you find in the link below.
...
private:
    ...
    ros::Publisher pubTimeM; 
    ros::Publisher pubTimeL; 
    ...
    pcl::PointCloud<PointType>::Ptr globalMapKeyFramesDS0;
...
public:
        ...
        nh("~")
    {
        ...
        pubHistoryKeyFrames = nh.advertise<sensor_msgs::PointCloud2>("/ekf_loam/history_cloud", 2);
        pubIcpKeyFrames = nh.advertise<sensor_msgs::PointCloud2>("/ekf_loam/corrected_cloud", 2);
        pubRecentKeyFrames = nh.advertise<sensor_msgs::PointCloud2>("/ekf_loam/recent_cloud", 2);

        pubTime = nh.advertise<std_msgs::Time>("/ekf_loam/mapOptmizationTime", 1);
        pubTimeA = nh.advertise<std_msgs::Time>("/ekf_loam/mapOptmizationTimeA", 1);
        pubTimeM = nh.advertise<std_msgs::Time>("/ekf_loam/mapOptmizationTimeM", 1);
        pubTimeL = nh.advertise<std_msgs::Time>("/ekf_loam/mapOptmizationTimeL", 1);
    ...
    void allocateMemory(){
        ...
        globalMapKeyFramesDS0.reset(new pcl::PointCloud<PointType>());
    ...
    void initialization(){
        ...
        // kdtrees
        downSizeFilterCorner.setLeafSize(leafsizemap, leafsizemap, leafsizemap);
        downSizeFilterSurf.setLeafSize(leafsizemap, leafsizemap, leafsizemap);
        //downSizeFilterOutlier.setLeafSize(leafsizemap, leafsizemap, leafsizemap); 

        downSizeFilterHistoryKeyFrames.setLeafSize(leafsizemap, leafsizemap, leafsizemap); // for histor key frames of loop closure 
        downSizeFilterSurroundingKeyPoses.setLeafSize(leafsizemap, leafsizemap, leafsizemap); // for surrounding key poses of scan-to-map optimization 

        downSizeFilterGlobalMapKeyPoses.setLeafSize(leafsizemap, leafsizemap, leafsizemap); // for global map visualization 
    ...
    void transformAssociateToMap(){
        ...
        // To alpha-beta filter
        for (int i = 0; i < 6; i++) {
		    transformAssMapped[i] = transformTobeMapped[i];
		}
    ...
    void laserCloudCornerLastHandler(const sensor_msgs::PointCloud2ConstPtr& msg){
        //ROS_INFO("Entrou na função laserCloudCornerLastHandler");
    ...
    void laserCloudSurfLastHandler(const sensor_msgs::PointCloud2ConstPtr& msg){
        //ROS_INFO("Entrou na função laserCloudSurfLastHandler");
    ...
    void visualizeGlobalMapThread(){
        ...
        pcl::io::savePCDFileASCII(fileDirectory+"finalCloud.pcd", *globalMapKeyFramesDS0);
    ...
    void publishGlobalMap(){
        ...
        pcl_ros::transformPointCloud(*globalMapKeyFramesDS, *globalMapKeyFramesDS0, pose);
        ...
        timeEndM = ros::Time::now().toSec();
        double timeDiffM = timeEndM - timeInitM;
        std_msgs::Time msgTimeDiffM;
        msgTimeDiffM.data = ros::Time().fromSec(timeDiffM);
        if (pubTimeM.getNumSubscribers() != 0){
            pubTimeM.publish(msgTimeDiffM);
        }
    ...
    void performLoopClosure(){
        ...
        // End time processing pointcloud 
        timeEndL = ros::Time::now().toSec();
        double timeDiffL = timeEndL - timeInitL;
        std_msgs::Time msgTimeDiffL;
        msgTimeDiffL.data = ros::Time().fromSec(timeDiffL);
        if (pubTimeL.getNumSubscribers() != 0){
            pubTimeL.publish(msgTimeDiffL);
        }
    }
    ...
    void run(){
        ...
        if (newLaserCloudCornerLast  && std::abs(timeLaserCloudCornerLast  - timeLaserOdometry) < refTime &&
           newLaserCloudSurfLast    && std::abs(timeLaserCloudSurfLast    - timeLaserOdometry) < refTime &&
           //newLaserCloudOutlierLast && std::abs(timeLaserCloudOutlierLast - timeLaserOdometry) < refTime &&
           newLaserOdometry)
        {
            newLaserCloudCornerLast = false; 
            newLaserCloudSurfLast = false; 
            ...
            if (timeLaserOdometry - timeLastProcessing >= mappingProcessInterval) {
                ...
                transformAssociateToMap(); // Associate transformation to Map

                extractSurroundingKeyFrames(); // Extract surround keyframes of pointcloud (corner, surf, and outlier)

                downsampleCurrentScan(); // downsample pointcoud (corner, surf, and outlier)

                scan2LiDARMapping();  // otimization pointcloud (corner, surf, and outlier) to Map

                saveKeyFramesAndFactor(); // Save key frame 
...
int main(int argc, char** argv)
{
    ...
    try
    { 
        nh_.param("/ekf_loam/fileDirectoryName", fileDirectory, std::string("/tmp/"));  
        nh_.param("/ekf_loam/leafsizemapValue", leafsizemap, float(0.16));
        nh_.param("/ekf_loam/globalMapVisualizationSearchRadiusValue", globalMapVisualizationSearchRadius, float(1000.0)); 
        nh_.param("/ekf_loam/enableLoopClosure", loopClosureEnableFlag, true); 
        nh_.param("/ekf_loam/keyframeRadiuns", historyKeyframeSearchRadius, float(7.0)); 
        nh_.param("/ekf_loam/keyframeNum", historyKeyframeSearchNum, 25); 
        nh_.param("/ekf_loam/keyframeScore", historyKeyframeFitnessScore, float(0.3));
        nh_.param("/ekf_loam/surroundingKeyframeSearchNumValue", surroundingKeyframeSearchNum, 50);
        nh_.param("/ekf_loam/surroundingKeyframeSearchRadiusValue", surroundingKeyframeSearchRadius, float(50.0)); 
    ...
    {
        ROS_INFO("\033[1;31m---->\033[0m Exception occurred when importing parameters in LiDAR Mapping Node. Exception Nr. %d", e);
    }
    ...
    std::thread loopthread(&LiDARMapping::loopClosureThread, &LM);
    std::thread visualizeMapThread(&LiDARMapping::visualizeGlobalMapThread, &LM);
